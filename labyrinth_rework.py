...
def render(board, xpos, length, ypos, height, curser, health, item_list, item_xpos, item_ypos):
    screen = []
    ...
    for row in range(ypos, ypos + height - 2):
        ...
        for column in range(xpos , xpos + length - 2):
            ...
            for item in item_list:
                if row == item_ypos[item] + (height//2) and column == item_xpos[item] + (length//2):
                    line += "H"
                    filled = True
...
def run(max_width, max_height, window_width, window_height):
    ...
    while (2 ** (hostile + 1)) < (((max_width**2) + (max_height**2))**(1/2))//1:
        hostile += 1
        item_list.append(hostile)

    playspace(space, max_width, max_height, xrange//2, yrange//2, clutter)
##    while check(space, xrange, yrange) == "failure":
##        playspace(space, max_width, max_height, xrange//2, yrange//2, clutter)

    for item in range(hostile + 1):
        item_x = random.randint(0, max_width)
        item_y = random.randint(0, max_height)
        while space[item_y + yrange//2][item_x + xrange//2] == clutter[1]:
            item_x = random.randint(0, max_width)
            item_y = random.randint(0, max_height)
#            if item_x > max_width//2:
#                item_x -= 1
...
#            if item_x == xrange//2:
#                if item_y < max_height//2:
#                    item_y += 1
#                if item_y > max_height//2:
#                    item_y -= 1

        item_xpos.append(item_x)
        item_ypos.append(item_y)


    while youre_winner == False and health > 0 :
        #print(curser * health)
        health = render(space, xpos, xrange, ypos, yrange, curser, health, item_list, item_xpos, item_ypos)
        global direction
